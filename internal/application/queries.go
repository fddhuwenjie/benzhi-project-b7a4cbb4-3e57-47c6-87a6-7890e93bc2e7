...
func (s *Service) buildVerificationPackage(ctx context.Context, projectID string) (*domain.VerificationPackage, domain.ManifestIntegrity, error) {
	...
	if err != nil {
		return nil, domain.ManifestIntegrity{}, verificationAuditLoadError(err)
	}
...
// verificationAuditLoadError adds a stable application error for audit lookup
// failures returned while assembling a verification package.
func verificationAuditLoadError(err error) error {
	if err == nil {
		return nil
	}
	return domain.ConflictWithDetails("发布批准审计读取失败", map[string]any{"cause": err.Error()})
}
