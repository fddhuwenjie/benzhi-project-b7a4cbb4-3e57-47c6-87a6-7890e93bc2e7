...
func (s *Service) SearchCues(ctx context.Context, projectID string, query domain.CueSearchQuery) (*domain.CueSearchResult, error) {
	...
	key := fmt.Sprintf("%s:%d:%s", projectID, project.Revision, encodedQuery)
	if cached, ok := s.searchCache[key]; ok {
		return cloneCueSearchResult(cached), nil
	}
	result, err := domain.SearchCues(project, query)
	if err != nil {
		return nil, err
	}
	if s.searchCache == nil {
		s.searchCache = map[string]*domain.CueSearchResult{}
	}
	s.searchCache[key] = result
	return cloneCueSearchResult(result), nil
}

func cloneCueSearchResult(input *domain.CueSearchResult) *domain.CueSearchResult {
	if input == nil {
		return nil
	}
	data, _ := json.Marshal(input)
	var output domain.CueSearchResult
