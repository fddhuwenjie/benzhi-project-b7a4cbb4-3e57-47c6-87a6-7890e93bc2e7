...
func (s *Service) AuditEventDetail(ctx context.Context, projectID string, eventID int64, include string, revision int64) (*domain.AuditEventDetail, error) {
	...
	project, err := s.repo.Get(ctx, projectID)
	if err != nil {
		return nil, newAuditDetailLoadError(projectID, "project", err)
	}
	event, err := s.repo.AuditEvent(ctx, projectID, eventID)
	if err != nil {
		return nil, newAuditDetailLoadError(projectID, "event", err)
	}
	...
	if include == "revision_diff" {
		...
		if err != nil {
			return nil, newAuditDetailLoadError(projectID, "revision_diff", err)
		}
	}
	if include == "manifest" {
		...
		if err != nil {
			return nil, newAuditDetailLoadError(projectID, "manifest", err)
		}
	}
	return result, nil
}
