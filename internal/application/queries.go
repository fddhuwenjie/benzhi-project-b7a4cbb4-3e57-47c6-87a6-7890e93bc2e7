...
func (s *Service) FindingEvidence(ctx context.Context, projectID, findingID string, expectedRevision int64) (*domain.FindingEvidence, error) {
	...
	return s.findingEvidence(ctx, project, *finding)
}
...
type auditDetailCacheKey struct {
	projectID string
	eventID   int64
	include   string
	revision  int64
...
func (s *Service) cachedAuditEventDetail(key auditDetailCacheKey) *domain.AuditEventDetail {
	s.auditDetailMu.Lock()
	defer s.auditDetailMu.Unlock()
	return s.auditDetailCache[key]
}

func (s *Service) cacheAuditEventDetail(key auditDetailCacheKey, detail *domain.AuditEventDetail) {
	s.auditDetailMu.Lock()
	defer s.auditDetailMu.Unlock()
	s.auditDetailCache[key] = detail
}
...
func (s *Service) AuditEventDetail(ctx context.Context, projectID string, eventID int64, include string, revision int64) (*domain.AuditEventDetail, error) {
	...
	cacheKey := auditDetailCacheKey{projectID: projectID, eventID: eventID, include: include, revision: revision}
	if cached := s.cachedAuditEventDetail(cacheKey); cached != nil {
		return cached, nil
	}
	project, err := s.repo.Get(ctx, projectID)
	if err != nil {
	...
	s.cacheAuditEventDetail(cacheKey, result)
	return result, nil
}
