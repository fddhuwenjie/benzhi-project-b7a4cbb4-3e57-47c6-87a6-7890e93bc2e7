...
func (s *Service) findingEvidence(ctx context.Context, project *domain.CaptionProject, finding domain.ReviewFinding) (*domain.FindingEvidence, error) {
	...
	read := captureFindingEvidenceRead(project, finding)
	...
	view := domain.CompareFindingEvidence(read.projectID, read.finding, reported, resolved, read.current)
	return &view, nil
}

type findingEvidenceRead struct {
	projectID string
	finding   domain.ReviewFinding
	current   *domain.CaptionCue
}

func captureFindingEvidenceRead(project *domain.CaptionProject, finding domain.ReviewFinding) findingEvidenceRead {
	read := findingEvidenceRead{projectID: project.ID, finding: finding}
