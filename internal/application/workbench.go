...
func (s *Service) GetWorkbench(ctx context.Context, projectID string) (*WorkbenchView, error) {
	if cached, ok := s.loadWorkbenchCache(projectID); ok {
		return cached, nil
	}
	...
	s.storeWorkbenchCache(projectID, view)
	return view, nil
}

func (s *Service) loadWorkbenchCache(projectID string) (*WorkbenchView, bool) {
	s.workbenchMu.RLock()
	data, ok := s.workbenchCache[projectID]
	data = append([]byte(nil), data...)
	s.workbenchMu.RUnlock()
	if !ok {
		return nil, false
	}
	var view WorkbenchView
...
func (s *Service) storeWorkbenchCache(projectID string, view *WorkbenchView) {
	data, err := json.Marshal(view)
	if err != nil {
		return
	}
	s.workbenchMu.Lock()
	s.workbenchCache[projectID] = data
