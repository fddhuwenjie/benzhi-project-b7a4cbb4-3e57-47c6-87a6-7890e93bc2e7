...
func (s *Service) GetWorkbench(ctx context.Context, projectID string) (*WorkbenchView, error) {
	...
	projectResults := make(chan projectResult, 1)
	auditResults := make(chan auditResult, 1)
	go func() {
		project, err := s.repo.Get(ctx, projectID)
		projectResults <- projectResult{project: project, err: err}
	}()
	go func() {
		events, err := s.repo.Audit(ctx, projectID, 0, 100)
		auditResults <- auditResult{events: events, err: err}
	}()
	projectRead, auditRead := <-projectResults, <-auditResults
	if projectRead.err != nil {
		return nil, projectRead.err
	}
	if auditRead.err != nil {
		return nil, auditRead.err
	}
	project, audit := projectRead.project, auditRead.events
	next := int64(0)
	if len(audit) > 0 {
	...
	if project.Status == domain.StatusReleased {
		manifest, manifestErr := s.ManifestReport(ctx, projectID)
		if manifestErr == nil {
