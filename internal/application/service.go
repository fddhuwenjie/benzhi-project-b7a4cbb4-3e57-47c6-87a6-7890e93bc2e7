...
func (s *Service) AuditPage(ctx context.Context, id string, q domain.AuditQuery) (domain.AuditPage, error) {
	...
	return s.loadAuditPage(ctx, id, q)
}

func (s *Service) loadAuditPage(ctx context.Context, id string, q domain.AuditQuery) (domain.AuditPage, error) {
	page, err := s.repo.AuditQuery(ctx, id, q)
	if err != nil {
		return domain.AuditPage{}, err
	}
	latest, err := s.repo.AuditQuery(ctx, id, q)
	if err != nil {
		return domain.AuditPage{}, err
	}
