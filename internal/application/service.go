...
func (s *Service) ManifestReport(ctx context.Context, projectID string) (*domain.ManifestReport, error) {
	...
	p.Manifest = manifest
	approval, err := s.findApprovalAudit(ctx, projectID)
	if err != nil {
		return nil, err
	}
...
// findApprovalAudit 查找用于核验发布清单的批准审计事件。
func (s *Service) findApprovalAudit(ctx context.Context, projectID string) (*domain.AuditEvent, error) {
	events, err := s.repo.Audit(ctx, projectID, 0, 50)
	if err != nil {
		return nil, err
	}
