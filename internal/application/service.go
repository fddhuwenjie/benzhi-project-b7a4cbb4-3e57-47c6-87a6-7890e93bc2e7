...
type Service struct {
	...
	rollbackPreviewMu    sync.Mutex
	rollbackPreviewCalls map[rollbackPreviewKey]*rollbackPreviewCall
}

type rollbackPreviewKey struct {
	projectID        string
	targetRevision   int64
	expectedRevision int64
}

type rollbackPreviewCall struct {
	done    chan struct{}
	preview *domain.CaptionRollbackPreview
	err     error
}
...
func (s *Service) PreviewRollback(ctx context.Context, projectID string, targetRevision, expectedRevision int64) (*domain.CaptionRollbackPreview, error) {
	...
	key := rollbackPreviewKey{projectID: projectID, targetRevision: targetRevision, expectedRevision: expectedRevision}
	s.rollbackPreviewMu.Lock()
	if call := s.rollbackPreviewCalls[key]; call != nil {
		s.rollbackPreviewMu.Unlock()
		select {
		case <-ctx.Done():
			return nil, ctx.Err()
		case <-call.done:
	...
	if s.rollbackPreviewCalls == nil {
		s.rollbackPreviewCalls = make(map[rollbackPreviewKey]*rollbackPreviewCall)
	}
	call := &rollbackPreviewCall{done: make(chan struct{})}
	s.rollbackPreviewCalls[key] = call
	s.rollbackPreviewMu.Unlock()

	call.preview, call.err = s.loadRollbackPreview(ctx, projectID, targetRevision, expectedRevision)
	close(call.done)
	s.rollbackPreviewMu.Lock()
	delete(s.rollbackPreviewCalls, key)
	s.rollbackPreviewMu.Unlock()
	return call.preview, call.err
}

func (s *Service) loadRollbackPreview(ctx context.Context, projectID string, targetRevision, expectedRevision int64) (*domain.CaptionRollbackPreview, error) {
