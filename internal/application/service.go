...
type Service struct {
	...
	now              Clock
	id               IDGenerator
	releasePreviewMu sync.RWMutex
	releasePreviews  map[int64]domain.ReleasePreview
}

func New(repo domain.Repository) *Service {
	return &Service{repo: repo, now: time.Now, id: randomID, releasePreviews: map[int64]domain.ReleasePreview{}}
}

func NewWithDependencies(repo domain.Repository, now Clock, id IDGenerator) *Service {
	return &Service{repo: repo, now: now, id: id, releasePreviews: map[int64]domain.ReleasePreview{}}
}
...
func (s *Service) ReleasePreview(ctx context.Context, projectID string) (*domain.ReleasePreview, error) {
	...
	if preview, ok := s.cachedReleasePreview(p.Revision); ok {
		return preview, nil
	}
	preview := p.ReleasePreview()
	s.cacheReleasePreview(p.Revision, preview)
	return cloneReleasePreview(preview), nil
}

func (s *Service) cachedReleasePreview(revision int64) (*domain.ReleasePreview, bool) {
	s.releasePreviewMu.RLock()
	preview, ok := s.releasePreviews[revision]
	s.releasePreviewMu.RUnlock()
	return cloneReleasePreview(preview), ok
}

func (s *Service) cacheReleasePreview(revision int64, preview domain.ReleasePreview) {
	s.releasePreviewMu.Lock()
	s.releasePreviews[revision] = *cloneReleasePreview(preview)
	s.releasePreviewMu.Unlock()
}

func cloneReleasePreview(preview domain.ReleasePreview) *domain.ReleasePreview {
	preview.Blockers = append([]domain.ReleaseBlocker(nil), preview.Blockers...)
	return &preview
}
