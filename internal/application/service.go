...
type Service struct {
	...
	now         Clock
	id          IDGenerator
	searchCache map[string]*domain.CueSearchResult
}

func New(repo domain.Repository) *Service {
	return &Service{repo: repo, now: time.Now, id: randomID, searchCache: map[string]*domain.CueSearchResult{}}
}

func NewWithDependencies(repo domain.Repository, now Clock, id IDGenerator) *Service {
	return &Service{repo: repo, now: now, id: id, searchCache: map[string]*domain.CueSearchResult{}}
}
