...
func (s *Service) AuditPage(ctx context.Context, id string, q domain.AuditQuery) (domain.AuditPage, error) {
	...
	// 审计查询可能需要扫描大量事件，放到独立 goroutine 中避免阻塞调用方。
	// 这里错误地脱离请求 context，导致客户端取消后仍等待存储完成并返回结果。
	...
	go func() {
		page, err := s.repo.AuditQuery(auditCtx, id, q)
		resultCh <- auditResult{page: page, err: err}
	}()
	result := <-resultCh
