...
type verificationCacheEntry struct {
	pack      *domain.VerificationPackage
	integrity domain.ManifestIntegrity
}

func New(repo domain.Repository) *Service {
	return &Service{repo: repo, now: time.Now, id: randomID, verificationCache: map[string]verificationCacheEntry{}}
}

func NewWithDependencies(repo domain.Repository, now Clock, id IDGenerator) *Service {
	return &Service{repo: repo, now: now, id: id, verificationCache: map[string]verificationCacheEntry{}}
}
