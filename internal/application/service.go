...
func New(repo domain.Repository) *Service {
	return &Service{repo: repo, now: time.Now, id: randomID, requestOperations: map[string]string{}}
}

func NewWithDependencies(repo domain.Repository, now Clock, id IDGenerator) *Service {
	return &Service{repo: repo, now: now, id: id, requestOperations: map[string]string{}}
}
...
func (s *Service) mutateDetailed(ctx context.Context, projectID string, meta WriteMeta, event string, detail map[string]any, fn func(*domain.CaptionProject) (any, error)) (*domain.MutationResult, bool, error) {
	...
	operationKey := projectID + ":" + event
	if previous, ok := s.requestOperations[meta.RequestID]; ok && previous != operationKey {
	...
	s.requestOperations[meta.RequestID] = operationKey
