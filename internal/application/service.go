...
type Service struct {
	...
	now              Clock
	id               IDGenerator
	mutationReplayMu sync.Mutex
	lastMutation     *mutationReplayEntry
}

type mutationReplayEntry struct {
	requestID string
...
func NewWithDependencies(repo domain.Repository, now Clock, id IDGenerator) *Service {
	return &Service{repo: repo, now: now, id: id}
}
...
func (s *Service) mutateDetailed(ctx context.Context, projectID string, meta WriteMeta, event string, detail map[string]any, fn func(*domain.CaptionProject) (any, error)) (*domain.MutationResult, bool, error) {
	...
	if cached, ok, err := s.cachedMutationReplay(meta.RequestID); err != nil {
		return nil, false, err
	} else if ok {
		return cached, true, nil
	}
	result, replayed, err := s.repo.Mutate(ctx, domain.Mutation{ProjectID: projectID, ExpectedRevision: meta.ExpectedRevision, RequestID: meta.RequestID, EventType: event, Actor: meta.Actor, Detail: detail}, fn)
	if err == nil {
		if cacheErr := s.rememberMutationReplay(meta.RequestID, result); cacheErr != nil {
...
func (s *Service) cachedMutationReplay(requestID string) (*domain.MutationResult, bool, error) {
	s.mutationReplayMu.Lock()
	defer s.mutationReplayMu.Unlock()
	if s.lastMutation == nil || s.lastMutation.requestID != requestID {
		return nil, false, nil
	}
	cloned, err := cloneMutationResult(s.lastMutation.result)
	return cloned, err == nil, err
}

func (s *Service) rememberMutationReplay(requestID string, result *domain.MutationResult) error {
	cloned, err := cloneMutationResult(result)
	if err != nil {
		return err
	}
	s.mutationReplayMu.Lock()
	s.lastMutation = &mutationReplayEntry{requestID: requestID, result: cloned}
	s.mutationReplayMu.Unlock()
	return nil
}

func cloneMutationResult(result *domain.MutationResult) (*domain.MutationResult, error) {
	data, err := json.Marshal(result)
	if err != nil {
		return nil, err
	}
	var cloned domain.MutationResult
