...
type Service struct {
	...
	id        IDGenerator
	diffMu    sync.Mutex
	diffCalls map[string]*revisionDiffCall
}

type revisionDiffCall struct {
	done   chan struct{}
	result *domain.RevisionDiff
	err    error
}

func New(repo domain.Repository) *Service {
	return &Service{repo: repo, now: time.Now, id: randomID, diffCalls: make(map[string]*revisionDiffCall)}
}

func NewWithDependencies(repo domain.Repository, now Clock, id IDGenerator) *Service {
	return &Service{repo: repo, now: now, id: id, diffCalls: make(map[string]*revisionDiffCall)}
}
...
func (s *Service) RevisionDiff(ctx context.Context, id string, from, to int64) (*domain.RevisionDiff, error) {
	...
	call := &revisionDiffCall{done: make(chan struct{})}
	s.diffCalls[key] = call
	s.diffMu.Unlock()

	call.result, call.err = s.loadRevisionDiff(ctx, id, from, to)
	s.diffMu.Lock()
	delete(s.diffCalls, key)
	close(call.done)
	s.diffMu.Unlock()
	return call.result, call.err
}

func (s *Service) loadRevisionDiff(ctx context.Context, id string, from, to int64) (*domain.RevisionDiff, error) {
