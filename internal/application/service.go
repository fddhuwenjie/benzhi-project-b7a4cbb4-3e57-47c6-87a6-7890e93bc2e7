...
func (s *Service) RevisionDiff(ctx context.Context, id string, from, to int64) (*domain.RevisionDiff, error) {
	...
	if err != nil {
		return nil, revisionDiffSnapshotError("起始", from, err)
	}
	tc, ts, err := s.repo.RevisionCues(ctx, id, to)
	if err != nil {
		return nil, revisionDiffSnapshotError("结束", to, err)
	}
	d := domain.CompareCueSnapshots(id, from, to, fc, tc, fs, ts)
	return &d, nil
}

// revisionDiffSnapshotError adds a user-facing stage for canceled reads.
func revisionDiffSnapshotError(stage string, revision int64, err error) error {
	if err == nil {
		return nil
	}
	...
	return fmt.Errorf("读取%s修订 %d 的字幕快照失败: %s", stage, revision, err.Error())
}
