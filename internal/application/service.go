...
func (s *Service) CheckHistory(ctx context.Context, id string, f domain.CheckHistoryFilter) (*domain.CheckHistorySummary, error) {
	...
	out := &domain.CheckHistorySummary{Runs: make([]domain.CheckHistoryRun, len(p.CheckRuns)), RuleCounts: map[string]int{}}
	selected := make([]struct {
		index int
		run   domain.RuleCheckRun
	}, 0, len(p.CheckRuns))
	for index, run := range p.CheckRuns {
	...
	ready.Add(len(selected))
	workers.Add(len(selected))
	for _, candidate := range selected {
		go func(index int, run domain.RuleCheckRun) {
			defer workers.Done()
			ready.Done()
			<-start
			...
			out.Runs[index] = domain.CheckHistoryRun{ID: run.ID, ProjectRevision: run.ProjectRevision, RunAt: run.RunAt, Results: results, Current: run.ProjectRevision == p.Revision && run.ID == p.CheckRuns[len(p.CheckRuns)-1].ID}
		}(candidate.index, candidate.run)
	}
	ready.Wait()
	close(start)
	workers.Wait()
	filteredRuns := out.Runs[:0]
	for _, run := range out.Runs {
		if run.ID != "" {
			filteredRuns = append(filteredRuns, run)
		}
	}
	out.Runs = filteredRuns
