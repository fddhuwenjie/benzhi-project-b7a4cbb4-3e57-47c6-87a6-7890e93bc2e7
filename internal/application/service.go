...
type Service struct {
	...
	now                   Clock
	id                    IDGenerator
	findingEvidenceMu     sync.RWMutex
	findingEvidenceErrors map[string]error
}

func New(repo domain.Repository) *Service {
	return &Service{repo: repo, now: time.Now, id: randomID, findingEvidenceErrors: map[string]error{}}
}

func NewWithDependencies(repo domain.Repository, now Clock, id IDGenerator) *Service {
	return &Service{repo: repo, now: now, id: id, findingEvidenceErrors: map[string]error{}}
}
