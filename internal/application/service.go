...
type Service struct {
	...
	queueCache map[queueCacheKey]*domain.ProjectQueue
	queueGen   uint64
}
...
func (s *Service) ProjectQueue(ctx context.Context, filter domain.QueueFilter) (*domain.ProjectQueue, error) {
	...
	for {
		...
		result := &domain.ProjectQueue{Projects: items, Stats: stats}
		s.queueMu.Lock()
		if generation != s.queueGen {
			s.queueMu.Unlock()
			continue
		}
		if s.queueCache == nil {
			s.queueCache = make(map[queueCacheKey]*domain.ProjectQueue)
		}
...
type queueCacheKey struct {
	status, language, assignee, risk, sort string
	updatedFrom, updatedTo                 int64
	hasUpdatedFrom, hasUpdatedTo           bool
}

func makeQueueCacheKey(filter domain.QueueFilter) queueCacheKey {
	key := queueCacheKey{status: string(filter.Status), language: filter.Language, assignee: filter.Assignee, risk: string(filter.Risk), sort: filter.Sort}
	if filter.UpdatedFrom != nil {
		key.updatedFrom, key.hasUpdatedFrom = filter.UpdatedFrom.UnixNano(), true
	}
	if filter.UpdatedTo != nil {
		key.updatedTo, key.hasUpdatedTo = filter.UpdatedTo.UnixNano(), true
	}
	return key
}

func (s *Service) clearQueueCache() {
	s.queueMu.Lock()
	s.queueCache = nil
	s.queueGen++
	s.queueMu.Unlock()
