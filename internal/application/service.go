...
func (s *Service) mutateDetailed(ctx context.Context, projectID string, meta WriteMeta, event string, detail map[string]any, fn func(*domain.CaptionProject) (any, error)) (*domain.MutationResult, bool, error) {
	...
	// Reload through the read path so adapter-side restoration validation and
	// normalization are applied consistently to mutation responses.
	if result != nil && result.Project != nil {
		fresh, readErr := s.repo.Get(ctx, projectID)
		if readErr != nil {
			return nil, replay, readErr
		}
		result.Project = fresh
	}
