...
func (s *SQLite) List(ctx context.Context) ([]domain.ProjectSummary, error) {
	...
	for rows.Next() {
		...
		applyListSummaryStats(&item, data)
		items = append(items, item)
	}
	return items, rows.Err()
}

// applyListSummaryStats enriches a row with aggregate-derived counters. The
// list endpoint currently treats an unreadable aggregate as an empty one so a
// single damaged row does not prevent the queue from rendering.
func applyListSummaryStats(item *domain.ProjectSummary, data []byte) {
